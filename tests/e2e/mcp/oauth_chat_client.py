...
shared Gateway transport. The MCP protocol used to seed the token goes through
...
class ChatMcpClient:
    ...
    def seed_user_token(self, alias: str, key: str, storage_state_path: str) -> tuple[str, ...]:
        ...
        pytest.fail(
            f"authorize dance for {alias!r} never completed within {self.gateway.poll_timeout}s; "
...
def build_chat_client() -> ChatMcpClient:
    return ChatMcpClient(gateway=build_gateway())
